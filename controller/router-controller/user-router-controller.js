...
const userRouterController = {
  ...
  showVerifyEmailPage: (req, res) => {
    if (!req.session.tempUserData)
      return res.redirect("/login?errMessage=Time is over, please try again");
    ...
    try {
      ...
      if (await otpController.verifyOTP(userData.email, req.body.otp)) {
        if (req.session.tempUserData.redirectUrl) {
          ...
          res.json({
            path: req.session.tempUserData.redirectUrl,
      ...
      if (err.message == "OTP EXPIRED") {
        ...
        res
          .status(440)
          .json({ errMessage: "Your OTP has expired, Please try again" });
  ...
  resendOtp: async (req, res) => {
    try {
      ...
      res
        .status(400)
        .json({ errMessage: "Something went wrong. Please try again later" });
    }
  },
  forgotPassword: async (req, res) => {
    try {
      ...
      req.session.tempUserData = {
        email,
        userId: user._id,
        redirectUrl: "/login/setNewPassword",
      };
      req.session.cookie.maxAge = 5 * 60000;
  ...
  setNewPassword: async (req, res) => {
    try {
      ...
      if (!req.session.tempUserData?.isOTPVerified)
        return res
          .status(400)
          .redirect("/login?errMessage=Email address is not verified");
  ...
  searchProducts: async (req, res) => {
    ...
    const {
      ...
      totalPages,
      currentPage,
    } = await productController.searchUserProducts(req.query);
  ...
  showAddressList: async (req, res) => {
    ...
    res.render("user/account/address-manage", {
      address,
      currentPath: req.url,
    });
  },
  updateUser: async (req, res) => {
    try {
      const newData = await userController.updateUser(
        req.session.user.userId,
        req.body
      );
  ...
  updateCart: async (req, res) => {
    ...
    try {
      const datas = await orderController.updateCart(
        req.session.user.userId,
        productId,
        quantity
      );
      ...
      const priceDetails = await orderController.getPriceSummary(
        req.session.user.userId
      );
      res.json({
      ...
      const currentQuantity = req.session.user.cart.find(
  ...
  verifyWalletPayment: (req, res) => {
    ...
    userController
      .addMoneyToWallet(userId, req.body)
  ...
  showWishlist: async (req, res) => {
    const products = await productController.getProductByIds(
  ...
  editAddressForm: async (req, res) => {
    ...
    try {
      const address = await userController.getAddressById(
        req.session.user.userId,
        addressId
      );
      res.render("user/account/address-form", {
      ...
      res
        .status(400)
        .redirect(
          `/account/address/add?errMessage=${err.message}&redirect=${req.query.redirect}`
        );
    }
  },
  ...
  deleteAddress: async (req, res) => {
    try {
      const response = await userController.removeAddress(
        req.session.user.userId,
        req.params.id
      );
      res.json(response);
  ...
  verifyPromocode: async (req, res) => {
    try {
      ...
      const couponResponse = await couponController.verifyCoupon(
        userId,
        promocode
      );
  ...
  showOrderDetails: async (req, res) => {
    try {
      ...
      const orderDetails = await orderController.getUserOrderDetails(
        userId,
        orderId
      );
  ...
  completePendingPayment: (req, res) => {
    orderController
      .completeOrderPayment(req.params.id)
      .then((response) => {
        req.session.orderDone = true;
        res.json(response);
      })
      .catch((err) =>
        res.status(err.statusCode || 400).json({ errMessage: err.message })
      );
  },
  addReview: async (req, res) => {
    try {
      const userId = req.session.user.userId;
      const orderId = req.params.id;
      const response = await orderController.addReview(
        userId,
        orderId,
        req.body
      );
  ...
  getOrderInvoicePDF: (req, res) => {
    ...
    const orderId = req.params.id;
    orderController
      .generateOrderInvoicePDF(userId, orderId)
